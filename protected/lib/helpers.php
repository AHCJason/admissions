...
function scheduleMenu($params, &$smarty) {
	...
	if (auth()->getRecord()->isAdmissionsCoordinator() == 1) {
		...
		/*} else*/
		if ($schedule->getPatient()->readyForApproval($msg, $schedule->getFacility()->id) && $schedule->status != 'Approved') {
			$options .= "\n<li><a class='approve-admit-link' href='{$SITE_URL}/?page=coord&action=room&amp;schedule={$schedule->pubid}&amp;goToApprove=1'>Approve this inquiry</a></li>";
		}
	...
	$options .= "\n<li><a href='{$SITE_URL}/?page=coord&amp;action=room&amp;schedule={$schedule->pubid}'>Manage Room Assignment</a></li>";
	...
	$options .= "\n<li id=\"record-visit\"><a href='{$SITE_URL}/?page=patient&amp;action=visit&amp;patient={$schedule->getPatient()->pubid}&schedule={$schedule->pubid}'>Record Patient Visit</a></li>";
